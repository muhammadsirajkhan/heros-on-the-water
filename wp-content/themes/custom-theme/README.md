...
| [`page-whats-on.php`](page-whats-on.php) | **What's On** page template: dual-month calendar + upcoming events (see **What's On page** below). |
...
- **Template**: Create a page in **Pages → Add New**, assign **What's On** (`page-whats-on.php`). Add it to a menu if needed.
- **Events CPT**: Registered by the theme in [`inc/post-type-event.php`](inc/post-type-event.php) (slug **`event`**, public URLs under **`/events/`**). If single event URLs 404 after install, open **Settings → Permalinks** and click **Save Changes** once.
- **ACF**: **Date Picker** **`event_date`** — strongly recommended **Return format `Ymd`** (e.g. `20260423`). Display format can stay human-readable. Return formats like **`F j`** (no year) are ambiguous; the theme normalizes common patterns and PHP-filters events, but **`Ymd` or `F j, Y`** avoids mistakes.
- **Time fields**: Optional **Time Pickers** **`event_start_time`** and **`event_end_time`** (e.g. return `g:i a`) show one line on cards (legacy `event_time` / `event_end` still work).
- **Logic**: [`inc/whats-on-events.php`](inc/whats-on-events.php) loads published events with an **`event_date` meta row**, normalizes each date to `Ymd`, then filters into the current+next month window for the grid and **`>=` today** for the upcoming list (sorted ascending).
- **UI**: Pill tabs **Upcoming events** / **Calendar view** (JS in [`assets/js/main.js`](assets/js/main.js)). Calendar uses **forest + pin** for today, **red + pin** for today when there is an event, **red only** for other event days. Place the pin graphic at [`assets/images/pin-calender.png`](assets/images/pin-calender.png) (filename spelling as shown).
...
- Added **What's On** page template (`page-whats-on.php`), event query helpers, and calendar / upcoming card partials.
- Registered **Events** (`event`) post type in `inc/post-type-event.php` so events appear in the admin sidebar without a separate CPT plugin.
- What's On: PHP-based event date filtering (works with more ACF return formats), calendar toggle UI, today / today+event / event-only cell styles, and optional pin image on today cells.
